...
@section('content')
		...
		<div class="page-header">
			...
			<div class="row">
              ...
              <h3 class="box-title">{{ trans('vehicle.create_page_heading') }}</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->            
            {!! Form::open(array('url' => 'vehicle/create')) !!}
              <div class="box-body">
                <div class="form-group">
                {!! Form::label('plate_number', trans('vehicle.field_plate_number')) !!}
                {!! Form::text('plate_number', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_plate_number') ]) !!}                 
                </div>   
                <div class="form-group">
                {!! Form::label('vehicle_type_id', trans('vehicle.field_vehicle_type')) !!}
                {!! Form::select('vehicle_type_id',$vehicle_type_options, '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_vehicle_type')]) !!}                 
                </div>   
                <div class="form-group">
                {!! Form::label('license_type_id', trans('vehicle.field_license_type')) !!}
                {!! Form::select('license_type_id',$license_type_options ,null, ['class' => 'form-control', 'multiple' => 'multiple','name' => 'license_type_id[]' ]) !!}                 
                </div>   
                 <div class="form-group">
                {!! Form::label('client_id', trans('vehicle.field_client')) !!}
                {!! Form::select('client_id',$client_options ,'', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_client')]) !!}                 
                </div>   
                <div class="form-group">
                {!! Form::label('registration_number', trans('vehicle.field_registration_number')) !!}
                {!! Form::text('registration_number', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_registration_number') ]) !!}                 
                </div>   
                <div class="form-group">
                {!! Form::label('owner_name', trans('vehicle.field_owner_name')) !!}
                {!! Form::text('owner_name', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_owner_name') ]) !!}                 
                </div>   
                <div class="form-group">
                {!! Form::label('owner_email', trans('vehicle.field_owner_email')) !!}
                {!! Form::text('owner_email', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_owner_email') ]) !!}                 
                </div>
                <div class="form-group">
                {!! Form::label('owner_phone', trans('vehicle.field_owner_phone')) !!}
                {!! Form::text('owner_phone', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_owner_phone') ]) !!}                 
                </div> 
                <div class="form-group">
                {!! Form::label('location', trans('vehicle.field_location')) !!}
                {!! Form::text('location', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_location') ]) !!}                 
                </div>
                <div class="form-group">
                {!! Form::label('chassis_number', trans('vehicle.field_chassis_number')) !!}
                {!! Form::text('chassis_number', '', ['class' => 'form-control', 'placeholder' => trans('vehicle.field_chassis_number') ]) !!}                 
                </div>                     
              </div>
              <!-- /.box-body -->
